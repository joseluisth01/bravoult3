...
class ReservasReportPDFGenerator
{
    ...
    private function get_turno_name($hora, $hora_vuelta)
    {
        ...
        $vuelta_formato = $hora_vuelta ? substr($hora_vuelta, 0, 5) : '00:00';
    ...
    private function add_table_header()
    {
        ...
        $this->pdf->Cell(45, 8, 'Servicio - Horario', 1, 0, 'C', true);
        $this->pdf->Cell(20, 8, 'Adultos', 1, 0, 'C', true);
        $this->pdf->Cell(15, 8, 'Niños', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Adultos Cord.', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Niños Cord.', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Total Desc.', 1, 0, 'C', true);
    ...
    private function add_origen_section($origen, $datos, &$total_turno)
    {
        ...
        // Nombre del origen
        $this->pdf->Cell(70, 5, '        ' . $origen, 0, 0, 'L');
        
        $totales = $datos['totales'];
        
        // Datos del origen
        $this->pdf->Cell(20, 5, number_format($totales['adultos']), 1, 0, 'C');
        $this->pdf->Cell(15, 5, number_format($totales['ninos']), 1, 0, 'C');
        $this->pdf->Cell(25, 5, number_format($totales['residentes']), 1, 0, 'C');
        $this->pdf->Cell(25, 5, number_format($totales['ninos_residentes']), 1, 0, 'C');
        $this->pdf->Cell(25, 5, number_format($totales['descuentos'], 2) . ' €', 1, 0, 'R');
        ...
        if (!empty($localizadores)) {
            ...
            $this->pdf->Cell(70, 4, '            ' . implode(', ', array_slice($localizadores, 0, 10)), 0, 1, 'L');
            
            // Si hay más de 10, mostrar en líneas adicionales
            $chunks = array_chunk($localizadores, 10);
            for ($i = 1; $i < count($chunks); $i++) {
                $this->pdf->Cell(70, 4, '            ' . implode(', ', $chunks[$i]), 0, 1, 'L');
            }
        }
    ...
    private function add_subtotal_row($label, $totales, $bold = false)
    {
        ...
        $this->pdf->Cell(70, 6, $label, 1, 0, 'R', true);
        $this->pdf->Cell(20, 6, number_format($totales['adultos']), 1, 0, 'C', true);
        $this->pdf->Cell(15, 6, number_format($totales['ninos']), 1, 0, 'C', true);
        $this->pdf->Cell(25, 6, number_format($totales['residentes']), 1, 0, 'C', true);
        $this->pdf->Cell(25, 6, number_format($totales['ninos_residentes']), 1, 0, 'C', true);
        $this->pdf->Cell(25, 6, number_format($totales['descuentos'], 2) . '€', 1, 0, 'R', true);
    ...
    private function add_totales_agencias($data)
    {
        ...
        $this->pdf->Cell(60, 8, 'Origen', 1, 0, 'L', true);
        $this->pdf->Cell(20, 8, 'Adultos', 1, 0, 'C', true);
        $this->pdf->Cell(15, 8, 'Niños', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Adultos Cord.', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Niños Cord.', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Total Desc.', 1, 0, 'C', true);
        $this->pdf->Cell(25, 8, 'Importe Total', 1, 0, 'C', true);
        $this->pdf->Cell(15, 8, 'Reservas', 1, 1, 'C', true);
        
        // Datos por agencia
        $this->pdf->SetFont('helvetica', '', 8);
        
        foreach ($data['totales_agencias'] as $origen => $totales) {
            $this->pdf->Cell(60, 6, $origen, 1, 0, 'L');
            ...
            $this->pdf->Cell(25, 6, number_format($totales['ninos_residentes']), 1, 0, 'C');
